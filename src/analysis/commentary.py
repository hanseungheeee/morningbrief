...
5. 종목 코멘트는 뉴스 근거가 있거나 등락이 큰 종목 위주로만 작성한다. 움직임이 미미하고
   ...
   ★ 근거 탐색 범위: 종목별 목록의 헤드라인뿐 아니라 그 아래 "· 요약"(summary)과
   [시장뉴스] 로 표시된 시장 전체 뉴스에서의 언급까지 모두 근거로 삼는다. 요약이나
   시장 뉴스에 움직임의 이유가 있으면 그것을 연결한다. 이 세 곳 어디에도 근거가 정말
   없을 때만 "관련 뉴스에서 확인되지 않음"이라고 쓴다. 단, [시장뉴스]가 그 종목을
   스치듯 언급만 할 뿐 등락과 무관하면 억지로 잇지 말라(억지 인과 금지는 그대로다).
6. 무버(movers)는 그날 크게 움직인 종목이므로 "왜 움직였나"의 뉴스 근거가 특히 중요하다.
   해당 종목 뉴스(헤드라인·요약·[시장뉴스])에서 원인이 확인되면 연결하고, 세 곳 모두에서
   원인이 확인되지 않을 때만 "급등/급락 원인은 관련 뉴스에서 확인되지 않음"이라고 쓴다.
   근거를 넓게 찾되, 관련 없는 뉴스를 원인으로 둔갑시키는 억지 해석은 금지.
...
_KEYS = ["market_summary", "index_comment", "macro_comment", "policy_comment", "calendar_comment",
         "crypto_comment", "key_topics", "stock_comments", "mover_comments"]


def _format_ticker_news(ticker_news: dict[str, list[dict]] | None) -> str:
    """{티커: [{headline, summary, from_market}]} 을 티커 단위 목록 텍스트로 만든다.

    헤드라인뿐 아니라 요약(summary)도 함께 실어 "왜 움직였나"의 근거를 넓힌다.
    시장 뉴스 교차검색으로 보강된 항목은 [시장뉴스] 로 표시해 개별 뉴스와 구분한다.
    """
    lines: list[str] = []
    for ticker, items in (ticker_news or {}).items():
        ...
        for it in items:
            tag = "[시장뉴스] " if it.get("from_market") else ""
            lines.append(f"  - {tag}{it.get('headline', '')}")
            summary = (it.get("summary") or "").strip()
            # 요약이 헤드라인과 사실상 같은 경우(짧은 general 뉴스)는 중복이라 생략
            if summary and summary.lower() not in it.get("headline", "").lower():
...
def _build_user_message(
    ...
    mover_news: dict[str, list[dict]] | None = None,
    ...
    return (
        ...
        "## 관심 종목별 뉴스 (헤드라인 + 요약, [시장뉴스]=시장 전체 뉴스 속 언급)\n"
        f"{_format_ticker_news(stock_news)}\n\n"
        "## 시장 무버 종목별 뉴스 (헤드라인 + 요약, [시장뉴스]=시장 전체 뉴스 속 언급)\n"
...
def generate_commentary(
    ...
    """시장 숫자 + 뉴스(+ 관심 종목·시장 무버·정책 뉴스·경제 지표)로 서사형 해설을 생성한다.
    ...
    for key in _KEYS:
        ...
        elif key == "calendar_comment":
            # 지표 없으면 null (템플릿이 섹션 해설을 숨김)
