...
STOCK_NEWS_DAYS: int = 3          # 최근 N일 기사만 조회 (촉매 놓침 줄이려 2→3)
STOCK_NEWS_MAX_PER_TICKER: int = 5  # 종목당 상위 N개 (해설 근거 확보 위해 3→5)
STOCK_NEWS_SUMMARY_MAX: int = 220   # 헤드라인에 덧붙일 기사 요약 최대 길이 (토큰 절약)
...
# 뉴스 요약 투입으로 해설이 길어져 2000 이면 JSON 이 잘려 파싱 실패 → 상향
